...
function SavedQueryList({
  ...
  const openNewQuery = () => {
    window.open(`${window.location.origin}/datadissection/sqllab?new=true`);
  };
  ...
  const openInSqlLab = (id: number) => {
    window.open(`${window.location.origin}/datadissection/sqllab?savedQueryId=${id}`);
  };
  ...
  const copyQueryLink = useCallback(
    (id: number) => {
      copyTextToClipboard(() =>
        Promise.resolve(
          `${window.location.origin}/datadissection/sqllab?savedQueryId=${id}`,
        ),
      )
        .then(() => {
