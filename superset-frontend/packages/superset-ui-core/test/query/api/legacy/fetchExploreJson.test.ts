...
describe('fetchExploreJson()', () => {
  ...
  it('returns a promise of LegacyChartDataResponse', () => {
    fetchMock.post('glob:*/datadissection/explore_json/', {
      field1: 'abc',
      field2: 'def',
    });
  ...
  it('uses GET when specified', async () => {
    expect.assertions(4);
    const mockUrl = 'glob:*/datadissection/explore_json/*';
    ...
    expect(mockCalls[0][0]).toEqual(
      'http://localhost/datadissection/explore_json/?form_data=%7B%22granularity%22%3A%22minute%22%2C%22viz_type%22%3A%22word_cloud%22%2C%22datasource%22%3A%221__table%22%7D',
    );
