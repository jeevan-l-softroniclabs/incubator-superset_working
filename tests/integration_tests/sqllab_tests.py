...
class TestSqlLab(SupersetTestCase):
    ...
    def test_sqllab_has_access(self):
        for username in ("admin", "gamma_sqllab"):
            self.login(username)
            for endpoint in ("/datadissection/sqllab/", "/datadissection/sqllab/history/"):
    ...
    def test_sqllab_viz(self):
        ...
        resp = self.get_json_resp("/datadissection/sqllab_viz/", data=data)
