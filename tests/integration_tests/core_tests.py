...
class TestCore(SupersetTestCase):
    ...
    def test_dashboard_endpoint(self):
        self.login()
        resp = self.client.get("/datadissection/dashboard/-1/")
    ...
    def test_save_slice(self):
        ...
        url = (
            "/datadissection/explore/table/{}/?slice_name={}&"
    ...
    def test_user_profile(self, username="admin"):
        ...
        resp = self.get_resp(f"/datadissection/profile/{username}/")
    ...
    def test_user_profile_optional_access(self):
        self.login(username="gamma")
        resp = self.client.get(f"/datadissection/profile/admin/")
        self.assertEqual(resp.status_code, 200)
    ...
    def test_slice_payload_no_datasource(self):
        self.login(username="admin")
        data = self.get_json_resp("/datadissection/explore_json/", raise_on_error=False)
    ...
    def test_explore_json_async_results_format(self):
        ...
        self.login(username="admin")
        rv = self.client.post(
            "/datadissection/explore_json/?results=true",
            data={"form_data": json.dumps(form_data)},
        )
        self.assertEqual(rv.status_code, 200)
    ...
    def test_explore_json_data(self, mock_force_cached, mock_cache):
        ...
        self.login(username="admin")
        rv = self.client.get("/datadissection/explore_json/data/valid-cache-key")
        data = json.loads(rv.data.decode("utf-8"))
    ...
    def test_explore_json_data_invalid_cache_key(self):
        ...
        rv = self.client.get(f"/datadissection/explore_json/data/{cache_key}")
        data = json.loads(rv.data.decode("utf-8"))
    ...
    def test_feature_flag_serialization(self):
        ...
        urls = [
            "/datadissection/sqllab",
            "/superset/welcome",
            f"/datadissection/dashboard/{dash_id}/",
            "/datadissection/profile/admin/",
    ...
    def test_dashboard_injected_exceptions(self, mock_db_connection_mutator):
        ...
        url = f"/datadissection/dashboard/{dash.id}/"
    ...
    def test_explore_redirect(self, mock_command: mock.Mock):
        ...
        rv = self.client.get(
            f"/datadissection/explore/?form_data={quote(json.dumps(form_data))}"
        )
        self.assertEqual(
